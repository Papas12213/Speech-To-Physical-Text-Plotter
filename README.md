...
3. Execution 
      - Now that your words have been turned into G-code, the G-code is sent to your plotter/machine. Since my plotter's dimensions are limited, it monitors the current x/y coordinates and compares them against the plotter's max dimensions. When the plotter finishes a word and notices it cannot fit the next word in the given space, it will pause and tell the user to lift the pen so that it can create a new line to continue the sentence. There were some bugs at this time, so I added a sort of “sleep” so that the motors could be done before it moved onto the next line.
